...
**Answer Q.1** In ecological forecasting, uncertainty refers to the fact that future ecosystem states cannot be predicted as a single exact value. The uncertainty is the set of possible alternate future conditions predicted by a model and the chance of observing each outcome.
...
**Answer Q.2** Knowing forecast uncertainty helps natural resource managers make risk-based decisions by clarifying how likely different outcomes are and how reliable the forecast is. If the uncertainty is too high, the manager may choose more robust and flexible actions.
...
Four-letter site identifier: BARC Latitude: 29.675982 Longitude: -82.008414 Lake area (km2): 0.13 Elevation (m): 27
...
**Answer Q.4** (1) increase algae concentrations (2) reduce dissolved oxygen.
...
**Answer Q.5** The air temperature and water temperature have a positive relationship at this lake. Because the air and water surface can have a thermal exchange, the pattern of the figure also indicates they have a positive relationship.
...
**Answer Q.6** The grey lines represent the ensemble of forecast simulations. Each grey line is one simulated forecast trajectory generated by the model under uncertainty estimation.
...
**Answer Q.7** The multiple linear regression model fits the data well. Except during rapid changes, the predicted water temperature is closed to the observed temperature.
...
**Answer Q.8** The fitted model coefficients indicate that current air temperature (0.81) has a much stronger influence on current water temperature than past water temperature (0.18). The standard errors of the parameters are relatively small for the predictors (0.0198 and 0.0185), indicating fair precision. The intercept has a larger standard error (0.4463), indicating greater uncertainty in that estimate.
...
**Answer Q.9** The model performance appears strong. The R² (0.9) indicates that the model explains about 90% of the variability in water temperature. The RMSE （0.61） suggests that the prediction error is relatively small. The extremely small err value indicates negligible numerical error.
...
**Answer Q.11** Yes, the forecasted water temperature generally follows the pattern of the forecasted air temperature because air temperature is an important driver in the model.
...
**Answer Q.12** As the forecast goes farther into the future, the uncertainty grows larger.
...
**Answer Q.13** The deterministic forecast provides a single predicted water temperature trajectory, while the probabilistic forecast shows multiple possible trajectories due to uncertainty in the air temperature driver. This indicates that water temperature could vary within a range during the week, rather than following one exact value.
...
**Answer Q.14** Parameter uncertainty could be reduced by selecting more observations in model or using more accurate measurement tools to improve the data quality.
...
**Answer Q.15** Initial conditions uncertainty could be reduced by improving the quality and completeness of observations.
...
**Answer Q.16** Parameter uncertainty.
...
*Driver uncertainty* The driver uncertainty would be the same. Because adding variables could not improve the precision of their quality.

*Parameter uncertainty* The parameter uncertainty would increase. Because adding more variables introduces more parameters that need to be estimated.

*Initial conditions uncertainty* The initial conditions uncertainty would be the same. Because it depends on the uncertainty in the starting water temperature observation.

*Process uncertainty* The process uncertainty would decrease. Because including additional environmental variables could allow the model to better represent processes affecting water temperature.
...
```{r}
fit <- lm(model_data$wtemp ~ model_data$wtemp_yday + model_data$airt + model_data$wnd + model_data$swr) #INSERT ADDITIONAL COVARIATES IN THE PARENTHESES
...
**Answer Q.19** The multiple linear regression model predicts current water temperature as a function of yesterday’s water temperature and several weather variables, including air temperature, shortwave radiation, and wind speed. Compared with the previous fitted model, this model indicates wind speed is negatively associated with water temperature (coefficient = -0.22), and short wave radiation has little impact on water temperature (coefficient = 0.00).
...
for(d in 2:length(forecasted_dates)) {
  ...
  #run model - REVISE THIS!!!!!!
    temp_pred$value <- coeffs[1] + temp_lag$value * coeffs[2] + temp_driv$air_temperature * coeffs[3] + temp_driv$wind_speed * coeffs[4] + temp_driv$shortwave_radiation * coeffs[5]
...
for(d in 2:length(forecasted_dates)) {
  ...
  #run the model using parameter distributions instead of fixed values - REVISE THIS!!
    temp_pred$value <- param.df$beta1 + temp_lag$value * param.df$beta2 + temp_driv$air_temperature * param.df$beta3 + temp_driv$wind_speed * param.df$beta4 + temp_driv$shortwave_radiation * param.df$beta5 
...
for(d in 2:length(forecasted_dates)) {
  ...
  #run model - REVISE THIS!!
    temp_pred$value <- coeffs[1] + temp_lag$value * coeffs[2] + temp_driv$air_temperature * coeffs[3] +  temp_driv$wind_speed * coeffs[4] + temp_driv$shortwave_radiation * coeffs[5] + rnorm(n = n_members, mean = 0, sd = sigma)
...
**Answer Q.25** Parameter uncertainty.
...
**Answer Q.26** Compare the amount of total forecast uncertainty over time: The total uncertainty decreased. Because the additional predictors (wind speed and shortwave radiation) improve the model’s explanatory power.

Compare the contribution of driver uncertainty: The contribution of driver uncertainty increased slightly in the updated model. Because the total model uncertainty decreased due to the process uncertainty, but the driver uncertainty didn't decrease as more parameters involved. So the contribution increased.

Compare the contribution of parameter uncertainty: The contribution of parameter uncertainty increases. Because with more variables incorporated, the observation of parameters cannot be improved.

Compare the contribution of process uncertainty: The contribution of process uncertainty decreased. Because more physical drivers of water temperature are included in the model, less variability remains unexplained.

Compare the contribution of initial conditions uncertainty: The contribution of initial condition uncertainty remained very small in both models. Because the influence of the starting water temperature weakens over time.
...
**Answer Q.27** I would use the money to improve the sensors to better measure the air temperature, water temperature, and wind speed. Because currently,the parameter uncertainty contributes the most of total forecast uncertainty.
